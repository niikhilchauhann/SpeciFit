...
3. Set up Firebase:
   ...
   - Run `flutterfire configure` in the root of the project to generate `lib/firebase_options.dart` and the respective `google-services.json` and `GoogleService-Info.plist` files for your project.
4. Get dependencies:
   ```bash
   flutter pub get
   ```
...
This project uses a `key.properties` file for Android release signing. This file is ignored in Git for security reasons.
If you wish to build a release version, you can either:
- Create your own keystore and `android/key.properties` file with your credentials.
- Or simply run a debug build (`flutter run`) which will use the default debug keystore.